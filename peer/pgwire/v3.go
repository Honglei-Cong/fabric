...
func parseOptions(data []byte) (SessionArgs, error) {
	args := SessionArgs{}
	buf := readBuffer{msg: data}
	for {
		key, err := buf.getString()
		if err != nil {
			return SessionArgs{}, fmt.Errorf("error reading option key: %s", err)
		}
		if len(key) == 0 {
			break
		}
		...
		if err != nil {
			return SessionArgs{}, fmt.Errorf("error reading option value: %s", err)
		}
		switch key {
		case "database":
		...
		default:
			logger.Warningf("unrecognized configuration parameter %q", key)
		}
	}
	return args, nil
}
...
func (c *v3Conn) setupSession(ctx context.Context) error {
	c.session = NewSession(ctx, c.sessionArgs, c.executor, c.conn.RemoteAddr())
	return nil
}
...
func (c *v3Conn) serve(ctx context.Context, draining func() bool) error {
	...
	c.conn = newReadTimeoutConn(c.conn, func() error {
		if err := func() error {
			if draining() && c.session.TxnState.State == NoTxn {
	...
	for {
		if !c.doingExtendedQueryMessage {
			...
			logger.Infof("pgwire: %s: %q", serverMsgReady, txnStatus)
...
func (c *v3Conn) sendAuthPasswordRequest() (string, error) {
	...
	if typ != clientMsgPassword {
		return "", fmt.Errorf("invalid response to authentication request: %s", typ)
	}
...
func (c *v3Conn) executeStatements(
	...
	results := c.executor.ExecuteStatements(c.session, stmts)
	defer results.Close()
		...
		case parser.Rows:
			...
			for rowIdx := 0; rowIdx < nRows; rowIdx++ {
				...
				for i, col := range row {
					...
					default:
						c.writeBuf.setError(fmt.Errorf("unsupported format code %s", fmtCode))
					}
				}
				if err := c.writeBuf.finishMsg(c.wr); err != nil {
					return err
...
func (c *v3Conn) sendRowDescription(
	ctx context.Context, columns []parser.ResultColumn, formatCodes []formatCode, canSendNoData bool,
) error {
...
func newUnrecognizedMsgTypeErr(typ clientMessageType) error {
	err := fmt.Errorf("unrecognized client message type %v", typ)
