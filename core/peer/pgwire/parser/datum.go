...
// *DOidWrapper is possible.
func AsDString(e Datum) (DString, bool) {
	switch t := e.(type) {
	case *DString:
		return *t, true
	case *DOidWrapper:
...
// assertion fails.
func MustBeDString(e Datum) DString {
	i, ok := AsDString(e)
	if !ok {
...
// *DOidWrapper is possible.
func AsDArray(e Datum) (*DArray, bool) {
	switch t := e.(type) {
	case *DArray:
		return t, true
	case *DOidWrapper:
...
// assertion fails.
func MustBeDArray(e Datum) *DArray {
	i, ok := AsDArray(e)
	if !ok {
...
func (d *DArray) Append(v Datum) error {
	if v != DNull && !d.ParamTyp.Equivalent(v.ResolvedType()) {
		return fmt.Errorf("cannot append %s to array containing %s", d.ParamTyp,
			v.ResolvedType())
	}
