...
func (b *writeBuffer) writeBinaryDatum(d parser.Datum, sessionLoc *time.Location) {
	...
	case *parser.DArray:
		...
		b.writeLengthPrefixedVariablePutbuf()
	default:
		b.setError(fmt.Errorf("unsupported type %T", d))
	}
}
...
func decodeOidDatum(id oid.Oid, code formatCode, b []byte) (parser.Datum, error) {
	...
	case formatText:
		...
		case oid.T_numeric:
			return nil, errors.New("T_numeric is not supported")
			//dd := &parser.DDecimal{}
		...
		case oid.T_bytea:
			...
			return nil, fmt.Errorf("unsupported bytea encoding: %q", b)
		case oid.T_timestamp:
			ts, err := parseTs(string(b))
			if err != nil {
				return nil, fmt.Errorf("could not parse string %q as timestamp", b)
			}
		...
		case oid.T_date:
			...
			if err != nil {
				...
				if err != nil {
					return nil, fmt.Errorf("could not parse string %q as date", b)
				}
				return res, nil
			}
		...
		case oid.T_interval:
			...
			if err != nil {
				return nil, fmt.Errorf("could not parse string %q as interval", b)
			}
			return d, nil
	...
	case formatBinary:
		...
		case oid.T_bool:
			...
			return nil, fmt.Errorf("unsupported binary bool: %x", b)
		case oid.T_int2:
			if len(b) < 2 {
				return nil, fmt.Errorf("int2 requires 2 bytes for binary format")
			}
			i := int16(binary.BigEndian.Uint16(b))
			return parser.NewDInt(parser.DInt(i)), nil
		case oid.T_int4:
			if len(b) < 4 {
				return nil, fmt.Errorf("int4 requires 4 bytes for binary format")
			}
			i := int32(binary.BigEndian.Uint32(b))
			return parser.NewDInt(parser.DInt(i)), nil
		case oid.T_int8:
			if len(b) < 8 {
				return nil, fmt.Errorf("int8 requires 8 bytes for binary format")
			}
			i := int64(binary.BigEndian.Uint64(b))
			return parser.NewDInt(parser.DInt(i)), nil
		case oid.T_oid:
			if len(b) < 4 {
				return nil, fmt.Errorf("oid requires 4 bytes for binary format")
			}
			u := binary.BigEndian.Uint32(b)
			return parser.NewDOid(parser.DInt(u)), nil
		case oid.T_float4:
			if len(b) < 4 {
				return nil, fmt.Errorf("float4 requires 4 bytes for binary format")
			}
			f := math.Float32frombits(binary.BigEndian.Uint32(b))
			return parser.NewDFloat(parser.DFloat(f)), nil
		case oid.T_float8:
			if len(b) < 8 {
				return nil, fmt.Errorf("float8 requires 8 bytes for binary format")
			}
		...
		case oid.T_numeric:
			...
			//		return nil, fmt.Errorf("could not parse string %q as decimal", decString)
			//	}
			...
			//default:
			//	return nil, fmt.Errorf("unsupported numeric sign: %d", alloc.pgNum.sign)
			//}
			//
			//return &alloc.dd, nil
		case oid.T_bytea:
		...
		case oid.T_timestamp:
			if len(b) < 8 {
				return nil, fmt.Errorf("timestamp requires 8 bytes for binary format")
			}
			i := int64(binary.BigEndian.Uint64(b))
			return parser.MakeDTimestamp(pgBinaryToTime(i), time.Microsecond), nil
		case oid.T_timestamptz:
			if len(b) < 8 {
				return nil, fmt.Errorf("timestamptz requires 8 bytes for binary format")
			}
			i := int64(binary.BigEndian.Uint64(b))
			return parser.MakeDTimestampTZ(pgBinaryToTime(i), time.Microsecond), nil
		case oid.T_date:
			if len(b) < 4 {
				return nil, fmt.Errorf("date requires 4 bytes for binary format")
			}
			i := int32(binary.BigEndian.Uint32(b))
	...
	default:
		return nil, fmt.Errorf("unsupported format code: %s", code)
	}
	...
	default:
		return nil, fmt.Errorf("unsupported OID %v with format code %s", id, code)
	}
}
...
func decodeBinaryArray(b []byte, code formatCode) (parser.Datum, error) {
	...
	if hdr.Ndims != 1 {
		return nil, fmt.Errorf("unsupported number of array dimensions: %d", hdr.Ndims)
	}
